-- Qualify folio columns that collide with modify_booking output variables.
begin;
...
begin
  ...
  if v_booking.status = 'checked_in' then
    ...
    update public.folio_charges fc set
      amount_ugx  = v_total,
      description = v_desc
    where fc.booking_id = p_booking_id
...
revoke all on function public.modify_booking(
  uuid, text, date, date, int, int, text, text, text, text, text
) from public;
